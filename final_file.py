    ...
    for result in response.results:
        ...
        for word in alternative.words:
           ...
           if '*' in word.word:
               ...
               end_time.append(word.start_time.seconds+word.start_time.nanos*(10**-9))
    return start_time,end_time,timeline

input_audio='aud_file.aac'

os.system(('ffmpeg -i {} -vn out1.flac ').format(input_audio))    #  for converting format
...
def split_profaned_audio(audio_path,start_time,end_time,ti_me):
    ...
    end_time=end_time
    audio_file="audio"+str(ti_me+1)+".flac"
    audio_file = r"%s"%audio_file
    os.system(('ffmpeg -i {} -ss {} -t {} -c copy {}').format(audio_path,start_time,end_time-start_time,audio_file))
    print('done')
   
audio_path='mono.flac'    
beep = open("beep.flac", "rb").read()
print('split0 ok')

split_profaned_audio(audio_path,timeline[0],start_time[0],-1)   #initialisation
#call(["ffmpeg","-i", "xyz1.wav", "-ss", "0", "-t", "start_time[1]", "-acodec", "copy", "sound0.wav"])
sound0 = open("audio0.flac", "rb").read()
combined_sounds = sound0 + beep
combined_sounds = open("combined_sounds.flac", "wb").write(combined_sounds)
combined_sounds= open("combined_sounds.flac", "rb").read()

'''
split_profaned_audio(audio_path,7.5,10,0)   #initialisation
#call(["ffmpeg","-i", "xyz1.wav", "-ss", "0", "-t", "start_time[1]", "-acodec", "copy", "sound0.wav"])

sound1 =  open("audio1.flac", "rb").read()
combined_sounds = combined_sounds+sound1+beep 
combined_sounds = open("combined_sounds2.flac", "wb").write(combined_sounds)
'''
if len_!=1:
    for ti_me in range(len_-1):
        split_profaned_audio(audio_path,end_time[ti_me],start_time[ti_me+1],ti_me)   #initialisation
        #call(["ffmpeg","-i", "xyz1.wav", "-ss", "0", "-t", "start_time[1]", "-acodec", "copy", "sound0.wav"])
        sound="audio"+str(ti_me+1)+".flac"
        sound = r"%s"%sound
        sound = open(sound, "rb").read()
        #sound = open("sound.flac", "wb").write(sound)
        combined_sounds = combined_sounds+sound + beep
        combined_sounds = open("combined_sounds.flac", "wb").write(combined_sounds)
        combined_sounds= open("combined_sounds.flac", "rb").read()
else: 
    ti_me=0
    split_profaned_audio(audio_path,end_time[ti_me],timeline[-1],ti_me)   #initialisation
    #call(["ffmpeg","-i", "xyz1.wav", "-ss", "0", "-t", "start_time[1]", "-acodec", "copy", "sound0.wav"])
    sound="audio"+str(ti_me+1)+".flac"
    sound = r"%s"%sound
    sound = open(sound, "rb").read()
